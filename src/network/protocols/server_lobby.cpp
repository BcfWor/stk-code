                                      ...
                                      std::shared_ptr<STKPeer> peer)
{
    ...
    else if ((argv[0] == "redteam") || (argv[0] == "redt") || (argv[0] == "rt") ||
		    (argv[0] == "blueteam") || (argv[0] == "bluet") || (argv[0] == "bt"))
    {
        std::string msg;
...
unmute_error:
    ...
    else if (argv[0] == "hangman" || argv[0] == "hm")
    {
	    handleHangmanCommand(peer, argv);
	    return;
    }
...
std::shared_ptr<STKPeer> ServerLobby::getPeerByPlayerName(const std::string& name)
{
    for (auto& peer : STKHost::get()->getPeers())
    {
        for (auto& player : peer->getPlayerProfiles())
        {
            if (StringUtils::toLowerCase(StringUtils::wideToUtf8(player->getName())) ==
...
void ServerLobby::handleHangmanCommand(std::shared_ptr<STKPeer> peer, const std::vector<std::string>& args)
{
    uint32_t peer_id = peer->getHostId();
    std::string player_name;
    for (auto& player : peer->getPlayerProfiles())
    ...
    if (args.size() < 2)
    {
        std::string msg = "Hangman commands: /hangman challenge [player], /hangman accept [player], /hangman guess [letter]";
        sendStringToPeer(msg, peer);
        return;
    }
    std::string cmd = StringUtils::toLowerCase(args[1]);
    if (cmd == "challenge" && args.size() >= 3)
    {
        std::string target_name = args[2];
        auto target_peer = getPeerByPlayerName(target_name);
        if (!target_peer)
        {
            std::string msg = "Player not found: " + target_name;
            sendStringToPeer(msg, peer);
            return;
        }
        uint32_t target_id = target_peer->getHostId();
        if (m_hangman_games.find(peer_id) != m_hangman_games.end())
        {
            std::string msg = "You already have an active Hangman game.";
            sendStringToPeer(msg, peer);
            return;
        }
        if (m_hangman_games.find(target_id) != m_hangman_games.end())
        {
            std::string msg = "That player already has an active Hangman game.";
            sendStringToPeer(msg, peer);
            return;
        }
        HangmanGame game;
        game.player1_id = peer_id;
        game.player2_id = target_id;
        game.accepted = false;
        game.wrong_guesses = 0;
        m_hangman_games[peer_id] = game;
        m_hangman_games[target_id] = game;
        std::string msg = "Hangman challenge sent to " + target_name;
        sendStringToPeer(msg, peer);
        std::string target_msg = player_name + " has challenged you to a game of Hangman! Type '/hangman accept " +
                         player_name + "' to accept.";
        sendStringToPeer(target_msg, target_peer);
    }
    else if (cmd == "accept" && args.size() >= 3)
    {
        std::string challenger_name = args[2];
        auto challenger_peer = getPeerByPlayerName(challenger_name);
        if (!challenger_peer)
        {
            std::string msg = "Player not found: " + challenger_name;
            sendStringToPeer(msg, peer);
            return;
        }
        uint32_t challenger_id = challenger_peer->getHostId();
        auto it = m_hangman_games.find(peer_id);
        if (it == m_hangman_games.end() || it->second.player1_id != challenger_id)
        {
            std::string msg = "No pending Hangman challenge from " + challenger_name;
            sendStringToPeer(msg, peer);
            return;
        }
        if (it->second.accepted)
        {
            std::string msg = "Game already started!";
            sendStringToPeer(msg, peer);
            return;
        }
        startHangmanGame(challenger_id, peer_id);
    }
    else if (cmd == "guess" && args.size() >= 3)
    {
        std::string guess_str = StringUtils::toLowerCase(args[2]);
        if (guess_str.length() != 1 || !std::isalpha(guess_str[0]))
        {
            std::string msg = "Please guess a single letter.";
            sendStringToPeer(msg, peer);
            return;
        }
        char guess = guess_str[0];
        auto it = m_hangman_games.find(peer_id);
        if (it == m_hangman_games.end() || !it->second.accepted)
        {
            std::string msg = "You don't have an active Hangman game.";
            sendStringToPeer(msg, peer);
            return;
        }
        if (it->second.current_turn != peer_id)
        {
            std::string msg = "It's not your turn to guess.";
            sendStringToPeer(msg, peer);
            return;
        }
        processHangmanGuess(peer_id, guess);
    }
    else
    {
        std::string msg = "Unknown Hangman command. Try: /hangman challenge [player], /hangman accept [player], /hangman guess [letter]";
        sendStringToPeer(msg, peer);
    }
}

void ServerLobby::startHangmanGame(uint32_t player1_id, uint32_t player2_id)
{
    if (m_jumble_word_list.empty())
    {
        Log::error("ServerLobby", "No words available for Hangman game!");
        return;
    }
    std::uniform_int_distribution<size_t> dist(0, m_jumble_word_list.size() - 1);
    std::string word = m_jumble_word_list[dist(m_jumble_rng)];
    HangmanGame& game1 = m_hangman_games[player1_id];
    HangmanGame& game2 = m_hangman_games[player2_id];
    game1.word = word;
    game1.guessed_letters.clear();
    game1.wrong_guesses = 0;
    game1.accepted = true;
    game1.current_turn = player1_id;
    game2.word = word;
    game2.guessed_letters.clear();
    game2.wrong_guesses = 0;
    game2.accepted = true;
    game2.current_turn = player1_id;
    auto peer1 = STKHost::get()->findPeerByHostId(player1_id);
    auto peer2 = STKHost::get()->findPeerByHostId(player2_id);
    if (peer1 && peer2)
    {
        std::string player1_name, player2_name;
        for (auto& player : peer1->getPlayerProfiles())
        {
            player1_name = StringUtils::wideToUtf8(player->getName());
            break;
        }
        for (auto& player : peer2->getPlayerProfiles())
        {
            player2_name = StringUtils::wideToUtf8(player->getName());
            break;
        }
        std::string start_msg = "Hangman game started between " + player1_name + " and " + player2_name + "!";
        sendStringToPeer(start_msg, peer1);
        sendStringToPeer(start_msg, peer2);
        std::string display = getHangmanDisplay(game1);
        sendStringToPeer(display, peer1);
        sendStringToPeer(display, peer2);
        std::string turn_msg = player1_name + "'s turn to guess a letter.";
        sendStringToPeer(turn_msg, peer1);
        sendStringToPeer(turn_msg, peer2);
    }
}

void ServerLobby::processHangmanGuess(uint32_t player_id, char guess)
{
    auto it = m_hangman_games.find(player_id);
    if (it == m_hangman_games.end() || !it->second.accepted)
        return;
    HangmanGame& game = it->second;
    if (game.guessed_letters.find(guess) != game.guessed_letters.end())
    {
        auto peer = STKHost::get()->findPeerByHostId(player_id);
        if (peer)
        {
            std::string msg = "Letter '" + std::string(1, guess) + "' already guessed!";
            sendStringToPeer(msg, peer);
        }
        return;
    }
    game.guessed_letters.insert(guess);
    bool correct = false;
    ...
    auto peer1 = STKHost::get()->findPeerByHostId(game.player1_id);
    auto peer2 = STKHost::get()->findPeerByHostId(game.player2_id);
    if (!peer1 || !peer2)
        return;
    std::string player1_name, player2_name;
    for (auto& player : peer1->getPlayerProfiles())
    ...
    std::string guesser_name = (player_id == game.player1_id) ? player1_name : player2_name;
    std::string guess_msg = guesser_name + " guessed '" + std::string(1, guess) + "': " +
                           (correct ? "Correct!" : "Wrong!");
    sendStringToPeer(guess_msg, peer1);
    sendStringToPeer(guess_msg, peer2);
    uint32_t other_id = (player_id == game.player1_id) ? game.player2_id : game.player1_id;
    auto other_it = m_hangman_games.find(other_id);
    if (other_it != m_hangman_games.end())
    {
        other_it->second.guessed_letters.insert(guess);
        if (!correct)
        {
            other_it->second.wrong_guesses++;
        }
    }
    bool word_completed = true;
    for (char c : game.word)
    {
        if (game.guessed_letters.find(c) == game.guessed_letters.end())
        {
            word_completed = false;
            break;
        }
    }
    std::string display = getHangmanDisplay(game);
    sendStringToPeer(display, peer1);
    sendStringToPeer(display, peer2);
    if (word_completed)
    {
        std::string win_msg = "Congratulations! The word was: " + game.word;
        sendStringToPeer(win_msg, peer1);
        sendStringToPeer(win_msg, peer2);
        m_hangman_games.erase(game.player1_id);
        m_hangman_games.erase(game.player2_id);
    }
    else if (game.wrong_guesses >= 6)
    {
        std::string lose_msg = "Game over! The word was: " + game.word;
        sendStringToPeer(lose_msg, peer1);
        sendStringToPeer(lose_msg, peer2);
        m_hangman_games.erase(game.player1_id);
        m_hangman_games.erase(game.player2_id);
    }
    else
    {
        ...
        other_it->second.current_turn = other_id;
        std::string next_player = (other_id == game.player1_id) ? player1_name : player2_name;
        std::string turn_msg = next_player + "'s turn to guess a letter.";
        sendStringToPeer(turn_msg, peer1);
        sendStringToPeer(turn_msg, peer2);
    }
}

std::string ServerLobby::getWordDisplay(const HangmanGame& game)
{
    std::string display;
    for (char c : game.word)
    {
...
std::string ServerLobby::getHangmanArt(int wrong_guesses)
{
    ...
    if (wrong_guesses >= 0 && wrong_guesses < (int)stages.size())
    {
        return stages[wrong_guesses];
    }
    return stages.back();
}
std::string ServerLobby::getHangmanDisplay(const HangmanGame& game)
{
    std::string art = getHangmanArt(game.wrong_guesses);
    std::string word_display = getWordDisplay(game);
    std::string guessed = "Guessed: ";
    for (char c : game.guessed_letters)
    {
        guessed += c;
        guessed += " ";
    }
