...
bool UnmuteCommand::execute(nnwcli::CommandExecutorContext* const ctx, void* const data)
{
    STK_CTX(stk_ctx, ctx);
    ...
    *parser >> playername; // Get arg 3
    ...
    auto mute_it = mutes.find(playername_w);
    if (mute_it == mutes.end())
    {
        ctx->write("Player is not muted.");
        ctx->flush();
        return false;
    }
    else
    {
        ctx->write("Unmuted player ");
